...
def train(output_dir, logging_dir, language, pre_trained_model_name, training_split, test_split):
    ...
    training_dataset_dir_path = os.path.join(dataset_cache_dir, "train")
    test_dataset_dir_path = os.path.join(dataset_cache_dir, "test")
    ...
    if Path(training_dataset_dir_path).is_dir() and Path(test_dataset_dir_path).is_dir():
        ...
        try:
            ...
            cv_dataset_config_file_path = os.path.join(dataset_cache_dir, 'dataset_config.txt')

            with open(cv_dataset_config_file_path, 'w') as f:
                f.write(CV_DATA_URL + "\n")
                f.write(CV_VERSION + '\n')
                f.write(CV_ID + '\n')
                f.write(training_split  + '\n')
                f.write(test_split  + '\n')

            shutil.copy(cv_dataset_config_file_path, output_dir)
        ...
        common_voice_test = load_dataset(dataset_name, language, split=test_split)
        ...
        with open('vocab.%s.tmp.json' % language, 'w') as vocab_file:
        ...
        print ("\nConstructing tokenizer")
        tokenizer = Wav2Vec2CTCTokenizer("./vocab.%s.tmp.json" % language, unk_token="[UNK]", pad_token="[PAD]", word_delimiter_token="|")
    ...
    #
    shutil.copy(os.path.join(dataset_cache_dir, 'dataset_config.txt'), os.path.join(output_dir, "/"))

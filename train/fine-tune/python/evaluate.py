...
def main(wav2vec2_model_path, revision, test_split_name, **args):
    evaluate(wav2vec2_model_path, revision, test_split_name)

def evaluate(wav2vec2_model_path, revision, test_split_name):
    ...
    #
    test_dataset = load_dataset("custom_common_voice.py", "cy", split=test_split_name)
    ...
    report = report + "WER: {:2f}\n".format(100 * wer.compute(predictions=result["pred_strings"], references=result["sentence"]))
    if ctcdecoder: report = report + "WER with CTC: {:2f}\n".format(100 * wer.compute(predictions=result["pred_strings_with_ctc"], references=result["sentence"]))
    if kenlm_ctcdecoder: report = report + "WER with CTC+LM: {:2f}\n".format(100 * wer.compute(predictions=result["pred_strings_with_lm"], references=result["sentence"]))

    report = report + "CER: {:2f}\n".format(100 * cer.compute(predictions=result["pred_strings"], references=result["sentence"]))
    if ctcdecoder: report = report + "CER with CTC: {:2f}\n".format(100 * cer.compute(predictions=result["pred_strings_with_ctc"], references=result["sentence"]))
    if kenlm_ctcdecoder: report = report + "CER with CTC+LM: {:2f}\n".format(100 * cer.compute(predictions=result["pred_strings_with_lm"], references=result["sentence"]))

    print (report)

    with(open(os.path.join(wav2vec2_model_path, "{}_results.txt".format(test_split_name)), 'w' )) as results_file:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--test-split-name", dest="test_split_name", default="test")

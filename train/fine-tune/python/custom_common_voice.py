...
try:
    #from train.python.cv_version_10_en_cy import CV_VERSION, CV_ID, CV_DATA_URL
    cv_config_import = os.environ['CV_CONFIG_FILE'] 
    mod = importlib.import_module(cv_config_import)
    CV_VERSION=mod.CV_VERSION
    CV_ID=mod.CV_ID
    CV_DATA_URL=mod.CV_DATA_URL
    print (CV_VERSION, CV_ID, CV_DATA_URL)
    del mod
except ImportError:
  print ("Please create a cv_version.py file. see cv_version.template.py")
...
class CustomCommonVoiceConfig(datasets.BuilderConfig):
    """BuilderConfig for Custom CommonVoice."""
    ...
    def __init__(self, name, sub_version, **kwargs):
        ...
        super(CustomCommonVoiceConfig, self).__init__(
            name=name, version=datasets.Version(CV_VERSION, ""), description=description, **kwargs
        )
...
class CustomCommonVoice(datasets.GeneratorBasedBuilder):
    ...
    def _split_generators(self, dl_manager):
        ...
        dl_path = dl_manager.download_and_extract(CV_DATA_URL.format(self.config.name))
        print(dl_path)
        abs_path_to_data = os.path.join(dl_path, CV_ID, self.config.name)
        ...
        validated_file_path = os.path.join(abs_path_to_data, "validated.tsv")
        if Path(validated_file_path).is_file():
            generated_splits.append(
                datasets.SplitGenerator(
                    name="validated",
                    gen_kwargs={
                            "filepath": validated_file_path,
                            "path_to_clips": abs_path_to_clips,
                    }
                )
            )

        other_file_path = os.path.join(abs_path_to_data, "other.tsv")
        if Path(other_file_path).is_file():
            generated_splits.append(
                datasets.SplitGenerator(
                    name="other",
                    gen_kwargs={
        ...
        invalidated_file_path = os.path.join(abs_path_to_data, "invalidated.tsv")
        if Path(other_file_path).is_file():
            generated_splits.append(
                datasets.SplitGenerator(
                    name="invalidated",
                    gen_kwargs={
                        "filepath": invalidated_file_path,
                        "path_to_clips": abs_path_to_clips,
                    }
                )
            )
        
        train_plus_file_path = os.path.join(abs_path_to_data, "train_plus.tsv")
        if Path(train_plus_file_path).is_file():
            generated_splits.append(
                datasets.SplitGenerator(
                    name="train_plus",
                    gen_kwargs={
                        "filepath": train_plus_file_path,
                        "path_to_clips": abs_path_to_clips,
                    }
                )
            )
        ...
        for s in range(2,4):
            for split_name in ["train","dev","test"]:
                split_s_file_path = os.path.join(abs_path_to_data, "{}.{}.tsv".format(split_name, s))            
                if Path(split_s_file_path).is_file():
                    generated_splits.append(
                        datasets.SplitGenerator(
                            name="{}_{}".format(split_name, s),
                            gen_kwargs={
                                "filepath": split_s_file_path,
                                "path_to_clips": abs_path_to_clips,
                            }
                        )
                    )
        ...
        for lang in _LANGUAGES.keys():
            lang_test_file_path = os.path.join(abs_path_to_data, "test-{}.tsv".format(lang))
            if Path(lang_test_file_path).is_file():
                generated_splits.append(
                    datasets.SplitGenerator(
                        name="test_" + lang,
                        gen_kwargs={
                            "filepath": lang_test_file_path,
                            "path_to_clips": abs_path_to_clips,
                        }
                    )
                )
